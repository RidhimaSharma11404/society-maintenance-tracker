...
async function runSeed() {
  ...
  // CLUSTER A: Tower B - 101 Plumbing (High risk cluster)
  ...
  // CLUSTER B: Tower A - 402 Electrical
  ...
  const overdue_complaint = {
    ...
    currentStatus: 'In Progress',
    createdAt: new Date(now - 2 * dayMs),
    dueDate: new Date(now - 1.7 * dayMs),
    statusHistory: [
      {
        status: 'Open',
        updatedBy: resident2._id,
  ...
  const allComplaints = [...clusterA_complaints, ...clusterB_complaints, overdue_complaint];
  ...
  console.log('[Seeder] Fixtures loaded successfully.');
}
